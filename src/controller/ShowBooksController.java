package controller;

import dataAccessObject.BooksDAOImpl;
import dataAccessObject.BooksDAOInterface;
import database.Database;
import model.Books;
import service.ServicePromotion;

import javax.servlet.RequestDispatcher;
import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import java.io.IOException;
import java.util.List;

@WebServlet(name = "ShowBooksController", urlPatterns = "/ShowBooksController")
public class ShowBooksController extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {

    }

    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        Database db = new Database();
        BooksDAOInterface dao = new BooksDAOImpl(db);
        List<Books> bList = dao.getAllBooks();
        List<Books> newPrice = ServicePromotion.discount(bList);
        db.close();
        RequestDispatcher rd = request.getRequestDispatcher("views/show_books.jsp");
        request.setAttribute("bList", newPrice);
        rd.forward(request, response);
    }
}
